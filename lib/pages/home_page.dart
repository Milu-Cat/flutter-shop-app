...
// import '../comments/bessel-curve.dart'; //贝塞尔曲线
import 'home_page/appbar_search.dart';
...
class _HomePageState extends State<HomePage>
  ...
  // 混入AutomaticKeepAliveClientMixin是为了保持页面状态
  @override
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: PreferredSize(
          ...
          child: AppBar(
            // title: Text('首页'),
            actions: <Widget>[AppBarSearch()],
            elevation: 0,
          ),
        ),
        body: FutureBuilder(
          future:
              DefaultAssetBundle.of(context).loadString("data/homePage.json"),
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              var data = json.decode(snapshot.data.toString());
              List<Map> swiper = (data['data']['slides'] as List).cast();
              List<Map> navigatorList =
                  (data['data']['topNavigator'] as List).cast();
              String adPicture = data['data']['adPicture']['image'];
              List<Map> recommendList =
                  (data['data']['recommend'] as List).cast();
              String floorImg = data['data']['floorTitle'];
              List<Map> floorContentList =
                  (data['data']['floor'] as List).cast();
              return EasyRefresh(
                child: SingleChildScrollView(
                  ...
                  children: <Widget>[
                    ...
                    FloorContent(
                      floorContentList: floorContentList,
                    ),
                    HotGoods(
...
class SwiperDiy extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      width: ScreenUtil().setWidth(1040.0),
      child: Swiper(
        itemBuilder: (BuildContext context, int index) {
          return InkWell(
              onTap: () {
                Application.router
                    .navigateTo(context, "/details?id=one"); // 跳到商品详情页
              },
              child: Container(
                  child: ClipRRect(
                child: Image.asset("${swiperDataList[index]['image']}",
                    fit: BoxFit.fill),
        ...
        children: <Widget>[
          Container(
            child: ClipOval(
              // 图片裁剪
              child:
                  Image.asset(item['image'], width: ScreenUtil().setWidth(95)),
            ),
          ),
          Text(
            item['topTitle'],
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: InkWell(
            onTap: () {
              Application.router
                  .navigateTo(context, "/goods-list?id=one"); // 跳到商品列表页
            },
...
class Recommed extends StatelessWidget {
  ...
  Widget _itemGoods(context, index) {
    return InkWell(
      onTap: () {
        Application.router
            .navigateTo(context, "/details-anther?id=two"); // 跳到商品详情页
      },
      child: Container(
    ...
    return Container(
        ...
        child: InkWell(
            ...
            child: Container(
              ...
              height: ScreenUtil().setHeight(200),
              child: Image.asset(floorImg, fit: BoxFit.fill),
            )));
  }
}
...
// 火爆标题
  ...
  Widget _hotGoods(BuildContext context) {
    return Container(
      child: Column(
        children: <Widget>[_hotTitle(), _hotContent(context)],
      ),
    );
  }
}
